...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        if len(conta.historico.transacoes_dia()) >= 10:
            print("\nUsuário excedeu o número de transações permitidas hoje!")
            return
...
def log_transaçao(func):
    def envelope(*args, **kwargs):
        resultado = func(*args, **kwargs)
        data_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        log_mesage = f"[{data_hora}] funçao '{func.__name__}' executada com os argumetnos {args} e {kwargs}. retornou {resultado}\n"
